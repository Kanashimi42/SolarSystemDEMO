...
public class LookAtTarget : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // making zoom through fov
        // mouse scroll delta y means vertical movement of mouse wheel, > 0 forward, < 0 backwards/to yourself
        float scroll = Input.mouseScrollDelta.y;

        if (scroll != 0)
        {
            Camera.main.fieldOfView -= scroll * 5f; //scroll parameter * speed of camera zoom
            Camera.main.fieldOfView = Mathf.Clamp(Camera.main.fieldOfView, 0.5f, 80f); // Mathf.Clamp keeps field of view between two parameters
        }
